...
import { autogfxApiRequest } from './GenericFunctions';
import { requestFields, requestOperations } from './RequestDescription';

export class Autogfx implements INodeType {
    description: INodeTypeDescription = {
        displayName: 'autoGfx',
        name: 'autogfx',
        icon: 'file:autogfx.svg',
        group: ['transform'],
        version: 1,
    ...
    async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
        ...
        for (let i = 0; i < items.length; i++) {
            try {
                ...
                const responseData = await autogfxApiRequest.call(this, requestMethod, endpoint, body);
